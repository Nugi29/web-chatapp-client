    ...
    <div className='h-full overflow-scroll relative backdrop-blur-lg'>
      ...
      <div className='flex items-center gap-3 py-3 mx-4 border-b border-stone-500'>
        ...
        <p className='flex-1 text-lg text-white flex items-center gap-2'>
          ...
          {onlineUser.includes(selectedUser._id) ? <span className='w-2 h-2 rounded-full bg-green-500'></span> : <span className='w-2 h-2 rounded-full bg-red-500'></span>}
        </p>
      ...
      <div className='flex flex-col h-[calc(100%-120px)] overflow-y-scroll p-3 pb-6'>
        ...
        {messages.map((msg, index) => (
          <div key={index} className={`flex items-end gap-2 justify-end ${msg.senderId !== authUser._id && 'flex-row-reverse'}`}>
            {msg.image ? (
              <img src={imagesDummyData[index]} alt="" className='max-w-[230px] border border-gray-700 rounded-lg 
